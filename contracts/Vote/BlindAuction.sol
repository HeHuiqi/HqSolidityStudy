...
contract BlindAuction {
        ...
        onlyBefore(revealEnd)
    {
        ...
        for (uint i = 0; i < length; i++) {
            Bid storage my_bid = bids[msg.sender][i];
            (uint value, bool fake, bytes32 secret) = (_values[i], _fake[i], _secret[i]);
            bytes memory ecdodeData = abi.encode(value,fake,secret);
                
            if (my_bid.blindedBid != keccak256(ecdodeData)) {
                // 出价未能正确披露
                // 不返还订金
                continue;
            }
            refund += my_bid.deposit;
            if (!fake && my_bid.deposit >= value) {
                if (placeBid(msg.sender, value))
                    refund -= value;
            }
            // 使发送者不可能再次认领同一笔订金
            my_bid.blindedBid = bytes32(0);
        }
